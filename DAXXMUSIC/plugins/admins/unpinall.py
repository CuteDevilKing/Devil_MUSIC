...
async def unpin_command_handler(client, message):
    ...
    if member.status == enums.ChatMemberStatus.ADMINISTRATOR or member.status == enums.ChatMemberStatus.OWNER:
        if member.privileges.can_pin_messages:
            ...
            msg_text = "You dont have permission to unpin something"
    ...
    await message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            [   
                [
                    InlineKeyboardButton(text="Yess", callback_data="unpinall=yes"),
                    InlineKeyboardButton(text="Nooo", callback_data="unpinall=no")
                ]
            ]
        )
    )
